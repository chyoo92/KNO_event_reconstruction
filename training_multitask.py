...
from datasets import dataset_main_multitask, dataset_test
...
def get_args_parser():
    ...
    parser.add_argument('--cla', action='store', type=int, default=8)
...
def main_one_gpu(args):
    ...
    if args.datasetversion == 0:
        dataset_module = dataset_main_multitask  #### main dataset code
    ...
    #### result folder
    result_path = 'result_multi/' + args.output
    ...
    if args.transfer_learning == 0:
        model = perceiver_i_multi(fea = args.fea, \
                        cla = args.cla, \
        ...
        checkpoint_path = f'{result_path}/checkpoint_94.pth'
    ...
    crit_logcosh = LogCoshLoss().to(device)
    crit_BCE = torch.nn.BCEWithLogitsLoss().to(device)
    ...
    for epoch in range(nEpoch):
        ...
        for i, batch_set in enumerate(tqdm(trnLoader, desc='epoch %d/%d' % (epoch+1, nEpoch))):
            ...
            vertex = vertex.float().to(device=device).reshape(-1,3)
            particle = particle.float().to(device=device).reshape(-1,1)
            direction = direction.float().to(device=device).reshape(-1,3)
            energy = (energy.float().to(device=device).reshape(-1,1))/100


            if padding_index is not None:
                pred1, pred2, pred3, pred4 = model(data,pmt_pos,padding_index)

            else:
                pred1, pred2, pred3, pred4 = model(data,pmt_pos)
            
            loss1 = crit_BCE(pred1, particle)
            loss2 = crit_logcosh(pred2, vertex)
            loss3 = crit_logcosh(pred3,direction)
            loss4 = crit_logcosh(pred4,energy)
            
            sigma1 = torch.exp(model.log_sigma1)
            sigma2 = torch.exp(model.log_sigma2)
            sigma3 = torch.exp(model.log_sigma3)
            sigma4 = torch.exp(model.log_sigma4)

            loss = (
            (1.0/(2.0 * sigma1**2)) * loss1 + torch.log(sigma1) +
            (1.0/(2.0 * sigma2**2)) * loss2 + torch.log(sigma2) +
            (1.0/(2.0 * sigma3**2)) * loss3 + torch.log(sigma3) +
            (1.0/(2.0 * sigma4**2)) * loss4 + torch.log(sigma4)
            )
            ...
            trn_acc += accuracy_score(particle.to('cpu'), np.where(pred1.to('cpu') > 0.0, 1, 0))*ibatch
        ...
        with torch.no_grad():
            for i, batch_set in enumerate(tqdm(valLoader)):
                ...
                val_acc += accuracy_score(particle.to('cpu'), np.where(pred1.to('cpu') > 0.0, 1, 0))*ibatch
